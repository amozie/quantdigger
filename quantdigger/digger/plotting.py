...
def plot_strategy(price_data, more_technicals={}, deals=[], curve=[], marks=[]):
    """
        显示回测结果。
    """
    max_key = max(more_technicals.keys())
    ...
    # 添加指标
    for key, values in more_technicals.items():
        if not isinstance(values, list):
            values = [values]
        for technicals in values:
